...
public class AddUserServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		 ...
		 * 
		 * (2)根据当前节点的层级数来获取它下面的两层子节点（12个子节点），根据左下标值进行升序排序
		 */
		...
		if(userService.addUser(user)) {
			...
			User current = userService.getUserByUserId(user_id);
			
			//更新：大于等于新增节点的左下标值的右下标值，都加个2
			userService.updateRight_num(user.getLeft_num(),current.getId()); 
			
			//更新：大于等于新增节点的右下标值的左下标值，都加个2
			userService.updateLeft_num(user.getRight_num(),current.getId()); 
			
			//获取该账号所在的树形结构的层级数
			int user_level = userService.getUserLevelByUserId(user_id);
			
			//更新该账号所在的树形结构的层级数
			userService.updateUser_level(user_level, user_id);
